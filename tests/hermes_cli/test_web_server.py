...
class TestWebServerEndpoints:
    ...
    def test_update_hermes_blocks_diverged_git_checkout_without_spawning(self, monkeypatch):
        import hermes_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("diverged git update guard should not spawn hermes update")
        ...
        assert data["name"] == "hermes-update"
        assert data["pid"] is None
        assert data["error"] == "git_update_not_applyable"
        assert data["message"] == "local repo diverged"
        assert spawned is False
    ...
    def test_update_hermes_uses_local_live_update_wrapper_when_available(self, monkeypatch):
        ...
        def fake_spawn(command, name, *, extra_env=None):
            calls.append((command, name, extra_env))
            return Proc()
        ...
        monkeypatch.setattr(web_server.secrets, "token_hex", lambda _n: "wrapper-action-id")
        web_server._ACTION_PROCS.pop("hermes-update", None)
        web_server._ACTION_RESULTS.pop("hermes-update", None)
        ...
        assert resp.json() == {
            ...
            "pid": 67890,
            "name": "hermes-update",
            "action_id": "wrapper-action-id",
        }
        assert calls == [
            (
                ["/home/alex/.local/bin/hermes-local-update"],
                "hermes-update",
                {"HERMES_ACTION_ID": "wrapper-action-id"},
            )
        ]
