    ...
    with write_txn(conn):
        ...
        if kind == "dependency":
            ...
            _append_event(
                ...
                _event_payload(kind=kind, source_status=source_status), run_id=run_id,
            )
        ...
        if recurrences >= BLOCK_RECURRENCE_LIMIT:
            ...
            _append_event(
                ...
                _event_payload(kind=kind, recurrences=recurrences, limit=BLOCK_RECURRENCE_LIMIT, source_status=source_status),
                run_id=run_id,
            )
        else:
            if expected_run_id is None:
            ...
            _append_event(
                ...
                _event_payload(kind=kind, recurrences=recurrences, source_status=source_status),
                run_id=run_id,
            )
        _blocked_task = get_task(conn, task_id)
    _fire_kanban_lifecycle_hook(
