...
def _spawn_detached_action(
    ...
    *,
    ...
    action_id = (extra_env or {}).get("HERMES_ACTION_ID")
    if action_id:
...
def _spawn_hermes_action(
    ...
    *,
    ...
    return _spawn_detached_action(cmd, name, extra_env=env_overrides)
...
async def update_hermes():
    """Kick off the supported update action in the background."""
    ...
    try:
        if install_method == "git":
            ...
            if applyability.get("spawn_mode") == "external":
                ...
                proc = _spawn_detached_action(
                    spawn_command,
                    "hermes-update",
                    extra_env={"HERMES_ACTION_ID": action_id},
                )
            else:
